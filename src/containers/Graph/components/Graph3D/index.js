...
export const Graph3D = ({
  ...
  const linkThreeObjectUpdate = (link) => {
    const verb = links.find(
      (_link) =>
        _link.source === link.source.id && _link.target === link.target.id
    );
    ...
    >
      ...
      {data && data.links && data.nodes && isLoaded && (
        <ForceGraph3D
          ...
          controlType="orbit"
          onNodeClick={(node) =>
            haveHandleClick ? handleClick(node) : () => {}
          }
          ...
          nodeThreeObject={(node) => {
            // eslint-disable-next-line
            const foundSourceLink = links.find(
              (_link) => _link.source === node.id
            );
            ...
            // Create a sphere
            const THEME =
              themeStatus === "dark" || themeStatus === "light"
                ? themeStatus
                : "dark";
            ...
            const sphere = new THREE.Mesh(
              ...
              new THREE.MeshBasicMaterial({
                color:
                  nodeColors[THEME][
                    foundSourceLink ? "sourceColor" : "subjectColor"
                  ],
                transparent: true,
                opacity: 0.95,
              })
