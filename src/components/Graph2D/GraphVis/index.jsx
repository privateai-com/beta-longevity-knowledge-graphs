...
export const GraphVis = memo(
  ({ nodes, edges, isEdit, setGraphData, setNodesLabelWithoutEdges }) => {
    ...
    const [, setShouldDeleteNode] = useState(false);

    const [, setModalDataFlag] = useState(false);
    ...
    useEffect(() => {
      ...
      const onClick = async (event) => {
        ...
        const canvasPosition = networkRef.current.canvasToDOM(
      ...
      if (networkRef.current)
        networkRef.current.on("hoverNode", () =>
          graphRef.current ? (graphRef.current.style.cursor = "help") : null
        );
      if (networkRef.current)
        networkRef.current.on("blurNode", () =>
          graphRef.current ? (graphRef.current.style.cursor = "default") : null
        );
      ...
      if (editingNodeId !== null) {
        ...
        if (updatedNode && inputRef.current) {
          ...
          const existingNode = nodes.get({
            filter: (node) =>
              node.label === newLabel && node.id !== editingNodeId,
          });
          ...
          if (newLabel) {
            currentEdges.forEach((edge) => {
              if (edge.id === updatedEdge.id) {
                ...
                const index = currentEdges.findIndex(
                  (e) => e.id === updatedEdge.id
                );
                if (index !== -1) {
        ...
        <div className={styles.visContainer}>
          ...
          >
            <div className={styles.popup_container}>
              ...
              />
              <Button
                className={styles.popup_button_ok}
